...
I wanted to keep the design minimalist which makes it more difficult to know how to control it. So this is an in-depht explation:
...
Also, it can have **exponential edit** which gives more precision for low values and less for big numbers.
...
> - **Drag with Left Click + Shift**: this gives 10 times more control, allowing for finer control.
...
<img src="screenshots/main_panel.png" width="350
" align="left">
 ...
 **Rings Fractal**: ${\Large z_{n+1} = \frac{z_n^2(z_n^2 + c)e^{2\pi\phi i}}{c z_n^2 + 1}}$
 ...
 where $z_n$ is each term of the sucesion, $c \in \mathbb{C}$,  $x$ and $\sigma$ are chosen complex numbers and $\phi = \frac{1 + \sqrt{5}}{2}$.


> [!NOTE]
> This formulas where adapted to allow full control over each thing
...
Apart from seeing if the number reaches a threshold, it can get in a loop, never reaching the threshold, which are the points that belong to the set. To solve this, theres is a limit on how many times the recursive formula can be fed into it self. 
...
When **Julia Set** is off, the initial $z$ value ($z_0$) is chosen, these are the default values:
- **Mandelbrot**: $z_0 = 0$
- **Burning Ship**: $z_0 = 0$
- **Rings Fractal**: $z_0 = 1$
...
In the other case, when **Julia Set** is on, the $c$ value is chosen, the defaults being:
...
And this time, is $z_0$ which is chosen based on the pixel that is being rendered.